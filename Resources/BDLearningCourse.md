...
```
Важно понимать, что почти все простые значения (целые, логические и др.) фиксированного размера и только строки - нефиксированного. Все значения фикисированного размера можно замещать другими значениями. Это делается через выделение поля и применения метода Set(), напр.: 
```
...
Все было гораздо лучше для случая наличия ключа. Тогда, для тех же действий по бинарному поиску, на каждом шаге поиска требовалось "заглядывать" только в одну таблицу. Чтобы получилось также, введем функцию с частичными свойствами ключевой функции. То есть, пусть на элементах опорной последовательности определена функция, выдающая сравнымый результат фиксированного размера. Например, на поле строкового типа можно вычислять целочисленное хеш-значение. Важно, чтобы функция максимально "разбрасывала" значения по диапазону. Почему полуключ? Потому что ключ дает взаимно однозначное соответствие между значением записи в последовательности и целочисленным кодом в каком-то интервале, а полуключ этого не делает. Разные записи могут давать одинаковые значения кода полуключа. 

Соответственно индексная последовательность строится на основе двухэтапной сортировки - сначала по полуключу, а потом по ключу опорной последовательности. Выигрыш заключается в том, что сравнение двух элементов, как правило, заканчивается на сравнении полуключей и только в случае совпадения, выполняется более сложное основное сравнение. 
...
Установить MongoDB можно с сайта https://www.mongodb.com. Я испытал, есть проблемы, но можно справиться. Желаю успеха...
```
Запуск сервера (правильно укажите директорию для данных или создайте какую-то по умолчанию)
```
mongod.exe --dbpath D:\WinApps\mongodata   
```
Обратите внимание, что сервер (сервис) ожидает запросов по некторым портам. Есть порт и для http-взаимодействия, можно попробовать, предварительно почитав документацию. Мы (с клиентом) будем пользоваться основным портом, сейчас он 27017 
...
После запуска клиента, можно что-то запрашивать и получать. Полезно заглянуть в help.
MongoDB для пользователя устроена довольно просто. Есть базы данных (dbs), есть коллекции (collections), в одной базе данный может быть сколько-то коллекций. Работу с базой данных надо определить указав use <db_name>. По этой команде произойдет создание базы данных или соединений с существующей. Для ориентировании в базах данных есть команда show dbs. Для ориентирования в коллекциях базы данных, есть команда show collections.
...
## Task08_JSON - минимальная работа с JSON/BSON
Рассмотренный ранее MongoDB использует JSON как формат структуризации данных при их хранении в базе данных. Но чаще, JSON используется как формат обмена структурированными данными. Обычно на запрос к серверу нужно возвратить ответ. Если результат структурированный, то нужен удобный формат. Часто используется JSON - Javascript Object Notation. Соответственно на стороне сервера или клиента может понадобиться сириализатор/десериализатор. В задании будет опробованы простые средства для работы с JSON. Мы не будем погружаться в сложности и тонкости, только поверхностный взгляд, остальное можно изучать по потребности, используя соответствующую литературу и программное обеспечение. 

JSON-объекты - это либо атомарные объекты, числа, строки, даты либо записи JSON-объектов, либо последовательности (массивы) JSON-объектов. Фигурные скобки применяются для записей, квадратные - для последовательностей. Таким образом, есть JSON-объекты и есть их текстовое представление. Соответственно, передача структурированных данных от одного агента к другому заключается в переводе JSON-объекта в текст (сериализация), передачи текста между агентами, обратном переводе из текста в объект (десериализация). В принципе, могут быть разные варианты внутреннего (объектного)представления таких данных, в предлагаемом подходе используются классы C# для записей JSON и списки для последовательностей. Соответственно структура
```
...
```
может быть сопоставлена текстовому значению:
```
{
...
```
И наоборот. Важно уметь эти преобразования выполнять тогда, когда нужно. А когда нужно? Предположим, мы создали объект класса Account в программе клиента. Поработали с ним, а теперь надо передать серверу (сервису). Напрямую объект не передается. Но можно превратить его в строку (сериализовать), передать серверу, а там преобразовать строку в новый объект. И снова можно работать с объектом, теперь уже на стороне сервера. 

В библиотеках существует разная поддержка JSON, мы воспользуемся библиотеками фирмы Newtonsoft. С помощью диспетчера NuGet загрузим пакет Newtonsoft.Json, а заодно и Newtonsoft.Bson, он нам понадобится позже.  

Все очень просто, преобразовать объект в строку можно статическим методом JsonConvert.SerializeObject:
```
    string json = JsonConvert.SerializeObject(account, Formatting.Indented);
    Console.WriteLine(json);
```
Обратное преобразование выполняется аналогично, только нужно еще указать класс объекта, в который преобразовывается JSON-строка:
```
    ...
    Console.WriteLine("\n");
```
Теперь попробуем сделать что-то похожее на базу данных. Определим два класса:
```
...
```
А в программе создадим вручную тестовую базу данных, сериализуем ее и посмотрим результат
```
...
```
Следующим шагом является имитация использования формата JSON для базы данных. Для этого, сгенерируем базу данных с числом персон, напр. 1 млн. Делается это через создание значения класса Database с помощью подходящего Linq-выражения, напр.:
```
    db = new Database
    {
...
```
Далее, сериализуем объект, сохраняем текст в файле (базы данных). Потом имитируем регулярную работу базы данных через начальное чтение из базы данных, десериализацию и строим цикл выборок персон по коду id.  

Характеристики по работе с базой данных JSON:
- 1 млн. персон
- запись базы данных 1.4 сек., объем файла 43.6 Мб.,
чтение базы данных 1.9, 1000 запросов по индексу 0.04 мс.

Сверхбыстрая выборка данных определяется тем, что в этот момент база данных развернута в оперативной памяти и тем, что доступ мы осуществляем просто индексируя кодом  массив. 

Вариант такой схемы организации базы данных заключается в замене формата JSON на BSON. BSON - бинарный формат хранения JSON-объектов. К сожалению, у меня не получилось выигрыша при перходе на этот формат, так что разбирать его не будем, желающие могут посмотреть написанный код и провести эксперименты. У меня все (!) характеристики ухудшились:

База данных BSON, 1 млн. элементов:
- запись базы данных 3.3 сек., объем файла 51.2 Мб.,
чтение базы данных 1.9, 1000 запросов по индексу - не измерял, должно быть как раньше.

### Task09_TCP - основы клиент-сервисной архитектуры, сетевое взаимодействие TCP
Будем выстраивать взаимодействие двух независимых программ. Одну назовем сервисом (по-привычке, я часто называю ее сервером), другую - клиентом. Базовым механизмом для сервиса является то, что будучи инициированным, сервис работает в ждущем режиме. Некоторый обработчик "вешается" на какой-то порт и запускается тогда, когда к порту посупает запрос извне. Этот ждущий объект называется Listener (слушатель). Вот с этого и начнем. Объявим пространства имен System.Net и System.Net.Sokets и начнем (в .NET Core) формировать сервис. 

Сетевое взаимодействие - процесс асинхронный, поэтому объявим асинхронный метод и организаем его использование:
```
...
// В программе:
    ...
    Console.ReadKey();
```
Будем разрабатывать метод StartServerAsync(). Нам нужен слушатель, его легко сделать через фрагмент:
```
TcpListener listener = new TcpListener(IPAddress.Parse("127.0.0.1"), 5000);
listener.Start();
``` 
Здесь происходит создание слушателя и установка его на IP-хост, известный как localhost и указанный порт 5000. После запуска слушателя, он будет готов в цикле принимать и исполнять запросы клиентов. Запросы клиентов обрабатываются следующим образом: 
- по каждому запросу формируется объект клиента,
- по клиенту берется поток (stream),   
- из потока читается собственно запрос,
- в поток пишется ответ.
Как-то так:
```
while (true)
{
...
``` 
Уже можно проверить работу программы. Мы ее запускаем. Если она правильно составлена, внешне она будет ждать нажатия на какую-нибудь кнопку клавиатуры. Но таже она будет ждать клиента с запросом по 5000 порту.  

Проимитируем клиента браузером. В любимом браузере наберите адресную строку http://localhost:5000
Скорее всего, сервер примет запрос и даже его выведет, а вот примет ли браузер ответ - вряд ли. Хотя есть и такие, что получают посланный сервером "OK". Дело в том, что браузер ожидает получения сообщения в регламентированном HTTP формате. Для Respons'а это что-то вроде:
``` 
HTTP/1.0 200 OK
...
Hello!
``` 
Измените переменную resp_message на это значение и у Вас, возможно, браузер получит желаемое Hello! Обратите внимание на то, что перед строчкой с данными (Hello!) есть пустая строка (символ \n), а Content-Length должен совпадать с числом байтов в данных. 

Подведем промежуточный итог. Мы построили некоторый сервис, который принимает произвольное (малое, до 1000 байтов) сообщение от клиента, посылает свое, в частности, в формате HTTP-ответа. После ответа, сервис разрывает соединение. Мы убедились в том, что сервис работает совместно со стандартным браузером. 

Следующая задача - создать клиента для этого сервиса. Мы работаем в технологии TCP и Socket. Эта технология позволяет выстравивать гибкие схемы распределенного взаимодействия. Можно реализовывать произвольный протокол, осуществлять обмены без разрыва соединения, передавать произвольные бинарные данные. В данном упражнении мы покажем частный случай клиент-сервисного взаимодействия, когда после соединения клиент передает сервису текстовое сообщение (request), сервис его принимает и отвечает отправкой назад другого текстового сообщения (response), которое принимается клиентом и происходит разрыв связи. Асинхронно и параллельно с этим сеансом связи, могут происходить и другие такие-же сеансы с другими копиями клиента. 

В чем основа TCP взаимодействия через Socket? На время соединения устанавливается канал связи, существенной частью когорого является двунаправленный поток байтов Stream. Со "своего" конца и клиент и сервис могут посылать блоки байтов и получать блоки байтов. Соответственно, порядок посылки и приема должен быть регламентирован каким-то стандартным или "самодельным" протоколом. Посланный или принятый информационный блок - это набор байтов, который будучи посланным, требует того, чтобы быть прочитанным. Посланные байты принимаются в том же порядке на приемной стороне. 

Создадим простого клиента. Это можно сделать на базе той же программы, которая при наличии аргумента "client" будет работать клиентом, при его отсутствии - сервисом. Клиент - также асинхронная программа. И в этом она похожа на сервис:
```
    ...
    // Клиентское асинхронное задание
    private static async Task<string> Ask(string request)
    {
    }
``` 
В клиентском задании, мы просто создаем объект клиента, коннектим его с нужным хостом и портом, далее - как для случая сервиса: берем стрим, пишем в него сообщение, читаем из него ответ от сервиса. Обратите внимание на то, как оформляется возврат результата для задания Task.
``` 
    private static async Task<string> Ask(string request)
    {
