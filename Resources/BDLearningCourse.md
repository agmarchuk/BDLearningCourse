...
=======
#### Асинхронные решения
Приведенные решения обладают существенным недостатком - они синхронны. Пока обрабатыватся один запрос, другие должны подождать. В этом можно убедиться "имитируя" длительную обработку, напр. вставив задержку:
```
System.Threading.Thread.Sleep(10000);
``` 
Величина задержки определяется в миллисекундах. Если между приемом команды и возвращением ответа вставить задержку, напр. в базовый вариант с разрывом соединения, то при почти одновременном запуске с двух консолей запросов на обработку, один ответ появится через 10 сек., второй - через 20.

Поэтому, в сетевых средствах TCP есть и асинхронные методы организации взаимодействий. Более того, они считаются основными, а синхронные дополнительными. Для того, чтобы понимать асинхронные программы, необходимо небольшое введение. 

В принципе, асинхронное программирование в C# было введено (на уровне библиотеки Framework) практически сразу. Это были Thread'ы ("нити"), средства управления ими и широкий набор примитивов синхронизации. Набор "классический", но не слишком удобный для работы, написание и отладка сложных программ, написанных в этом базисе требуют высокой квалификации именно в асинхронном программировании. С некоторых пор, появились более удобные средства, объединенные общей моделью. Более того, эти средства получили воплощение и в языке. Будем их изучать и использовать. Это не отменяет серьезное изучение технологии асинхронного программирования для тех, кому это нужно по сути выполняемых проектов.

Как обычно выполняется программа? Рассмотрим фрагмент:
```
...
MethodA(...);
...
```
Исполнение идет линейно по коду, подходит к методу, "заходит" в метод, завершает метод и выходит из него, выполняет следующие за методом действия. Для более ясного понимания сути следующих действий, заведем вспомогательную программу. В текущем проекте это можно сделать переименовав имеющийся Main() на что-нибудь вроде Main0() и сформировав новый Main() в другом месте, напр.:
```
using System;
using System.Threading.Tasks;
...
```
Что здесь важно? Ну, для будущего нам потребуется пространство имен System.Threading.Task, чтобы имитировать задержку обработке, в методе MethodA поставили цикл на миллиард повторов простого действия. Запуск программы поддтверждает наше представление о том, что запрограммировано. Теперь зададимся вопросом: а можно ли запустить исполнение метода и, не дожидаясь его окончания, продолжить работу основного потока управления. Ответ - можно, но надо несколько реорганизовать метод и по-другому выполнять его запуск. Изменения простые и не слишком ухудшают восприятие программы. 

Первая часть модификации программы - изменение вызова метода. Метод вызывается не "напрямую", а через специальное средство:
```
    Task.Run(()=>MethodA());
```
Можно сказать, что это "косвенный" запуск метода. Конструкция () => MethodA() - это лямбда-выражение.
Особенностью этого запуска является то, что запуск выполняется в новом Thread'е и выполянется (псевдо) параллельно основному исполнению. Сделав изменение и запустив программу, мы обнаружим, что метод (ради которого все и затевалось!) не исполнился. Причина простая - метод "пошел" на формирование исполнения, а управление в программе "пошло" дальше, добралось до оператора вывода, сделало его, добралось до конца, и все (процессы, нити и др.), подчиненные главному процессу "загасило". Для исправления ситуации, достаточно подождать. Для этого, в Main() перед выходом мы можем поставить ожидание ввода с клавиатуры
```
    Console.ReadKey();
```
Вставив ожидание, мы получили некоторый результат. Распчатка показывает, что все действия основного потока управления были выполнены раньше, чем (медленные) действия дополнительного потока, в котором исполнялся наш метод.

Теперь попробуем выполнить много раз тестовый метод, просто оргиназуем цикл типа:
```
    for (int i=0; i<10; i++) Task.Run(() => MethodA()); 
```
Убеждаемся, что программа работает. Причем, если снабдить запуски уникальными метками, то видно, что исполнение разных асинхронных ветвей выполняется не обязательно в порядке запуска. Это нормально. 

Однако, зададимся вопросом: а как зафиксировать, что все ветви уже исполнились? Для этого и для других целей, существует специальное оформление асинхронных запусков в виде так называемых заданий (Task). Мы уже незаметно "прошли" задания когда оформляли исполнение Task.Run(...). Этот метод имеет своим результатом объект типа Task. С этим объектом можно "работать". Например, сформировав массив запущенных заданий, можно дождаться когда они закончатся либо все, либо какое-то, подождем всех:
```
    int ntasks = 10;
...
```
D
Минимальное преобразование описания метода в асинхронных, затрагивает только строку заголовка:
```
    private static async Task MethodA()
```
появляется указание асинхронности и, очень важно, результат будет не void, а Task. 

Через такую трансформацию метод, при исполнении, как-бы разбивается на 2 действия. На формирование задания (Task) и на собственно исполнение задание. Причем эти действия выполняются в разных Thread'ах (нитях, потоках, процессах), т.е. асинхронно. Первое действие выполняется очень быстро, второе - столько сколько понадобится. Заметим, что тело метода не изменяется. 

Произведем в тестовой программе это изменение. 
>>>>>>> 47c8842f218df18834234b912d67149c3eb72d17
