...
Будем выстраивать взаимодействие двух независимых программ. Одну назовем севисом (по-привычке, я часто называю ее сервером), другую - клиентом. Базовым механизмом для сервиса является то, что будучи инициированным, сервис работает в ждущем режиме. Некоторый обработчик "вешается" на какой-то порт и запускается тогда, когда к порту посупает запрос извне. Этот ждущие метод называется Listener (слушатель). Вот с этого и начнем. Объявим пространства имен System.Net и System.Net.Sokets и начнем (в .NET Core) формировать сервис. 
