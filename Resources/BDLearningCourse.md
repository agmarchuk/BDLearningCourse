...
Задание попробуем выполнить вспоминая некоторую часть пройденного пути. Создадим в рабочей области проект дестопного C# Core приложения. Поставим его как главного. Проверим. 

Первый шаг - создание генератора тестовых данных. Нам нужна процедура, создающая поток пар {строка, число}. Сделаем эту процедуру по-новому, в стиле фукционального программирования, который есть в С#, а потом проверим его работу:
```
...
```
    ...
    bw.Flush(); fs.Flush();
```
Процесс чтения (сканирования) безы данных - очень похожий. Возвращаем головку чтения на начало, создаем бинарный ридер, читаем количество записей, читаем записи в указанном количестве. 
```
...
edge = {id1, id2}. 
```
Множество всех ребер в точности соответствует графу. Соотвественно, если граф задан как набор ребер (таких пар)
```
graph = [e1, e2, ...]
```
...
```
Снова появляется тройка. Первый вид ребра назывем объектное отношение (Object Property), второй - атрибутом (Datatype Property). В тройке первый аргумент называется субъект, второй - предикат, третий - объект. 
