...
```
Этот фрагмент определяется как тело определения функции (без результата)Ю на вход которой подается контекс, в частности Request (запрос) и его параметры, а косвенным результатом является Response, отправляемый браузеру. 

Теперь сформулируем то, что мы хотим, чтобы серверное приложение делало. В качестве постановки задачи мы видим следующее: если   
